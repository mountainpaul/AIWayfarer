...
def test_returns_503_when_google_not_configured():
    ...
    assert "Google token not found" in r.json()["message"]
...
def test_returns_502_on_calendar_api_error():
    ...
    assert "upstream 500" in r.json()["message"]
...
def test_happy_path_returns_normalized_events():
    ...
    with patch.object(google_auth, "get_credentials", return_value=MagicMock()), \
        ...
        r = client.get("/api/v1/calendar/events?from=2026-03-01T00:00:00Z&to=2026-04-30T00:00:00Z")
...
def test_rejects_invalid_iso_datetime():
    r = client.get("/api/v1/calendar/events?from=not-a-date")
    assert r.status_code == 400
    assert "invalid ISO datetime" in r.json()["message"]
