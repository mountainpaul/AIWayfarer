...
def test_delete_is_soft_and_hidden_but_recoverable():
    ...
    assert client.delete(f"/api/v1/bookings/{bid}").status_code == 204
    # Hidden from the normal read paths…
    assert client.get(f"/api/v1/bookings/{bid}").status_code == 404
    assert all(x["id"] != bid for x in client.get("/api/v1/bookings").json())
    # …but still present (recoverable) and tombstoned in the sync snapshot.
    snap = client.get("/api/v1/sync/snapshot").json()
...
def test_double_delete_is_404():
    ...
    assert client.delete(f"/api/v1/bookings/{bid}").status_code == 204
    assert client.delete(f"/api/v1/bookings/{bid}").status_code == 404


def test_cannot_update_a_deleted_booking():
    bid = _make_booking()["id"]
    client.delete(f"/api/v1/bookings/{bid}")
    assert client.patch(f"/api/v1/bookings/{bid}", json={"notes": "x"}).status_code == 404
...
def test_task_and_packing_delete_are_soft():
    ...
    leg = _a_leg_id()
    t = client.post(
        "/api/v1/tasks", json={"leg_id": leg, "title": "td", "priority": "low"}
    ).json()
...
def test_update_bumps_updated_at():
    ...
    before = b["updated_at"]
    after = client.patch(
        f"/api/v1/bookings/{b['id']}", json={"notes": "changed"}
    ).json()["updated_at"]
    assert after >= before
    client.delete(f"/api/v1/bookings/{b['id']}")  # cleanup: keep shared-DB counts stable
