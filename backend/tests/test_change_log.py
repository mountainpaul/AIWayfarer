...
def _leg_id() -> str:
    return client.get("/api/v1/legs").json()[0]["id"]
...
def _changes_for(entity_id: str) -> list[dict]:
    return client.get("/api/v1/changes", params={"entity_id": entity_id}).json()
...
def test_every_mutation_is_logged():
    b = _make_booking()
    bid = b["id"]
    client.patch(f"/api/v1/bookings/{bid}", json={"notes": "edited"})
    client.delete(f"/api/v1/bookings/{bid}")
...
def test_update_change_records_prior_value():
    ...
    bid = b["id"]
    client.patch(f"/api/v1/bookings/{bid}", json={"notes": "after"})
...
def test_undo_update_restores_overwritten_value():
    ...
    bid = b["id"]
    client.patch(f"/api/v1/bookings/{bid}", json={"notes": "WRONG"})
    assert client.get(f"/api/v1/bookings/{bid}").json()["notes"] == "WRONG"

    upd = [c for c in _changes_for(bid) if c["op"] == "update"][-1]
    r = client.post(f"/api/v1/changes/{upd['change_id']}/undo")
    assert r.status_code == 200
    assert client.get(f"/api/v1/bookings/{bid}").json()["notes"] == "pickup 2pm"
    _cleanup(bid)
...
def test_undo_delete_restores_row():
    ...
    bid = b["id"]
    client.delete(f"/api/v1/bookings/{bid}")
    assert client.get(f"/api/v1/bookings/{bid}").status_code == 404

    dele = [c for c in _changes_for(bid) if c["op"] == "delete"][-1]
    client.post(f"/api/v1/changes/{dele['change_id']}/undo")
    assert client.get(f"/api/v1/bookings/{bid}").status_code == 200  # back from the dead
    _cleanup(bid)
...
def test_undo_create_removes_the_row():
    ...
    client.post(f"/api/v1/changes/{crt['change_id']}/undo")
    assert client.get(f"/api/v1/bookings/{bid}").status_code == 404  # already hidden, no cleanup


def test_redo_via_undoing_the_undo():
    b = _make_booking(notes="v1")
    bid = b["id"]
    client.patch(f"/api/v1/bookings/{bid}", json={"notes": "v2"})
    upd = [c for c in _changes_for(bid) if c["op"] == "update"][-1]

    # Undo: back to v1.
    undo = client.post(f"/api/v1/changes/{upd['change_id']}/undo").json()
    assert client.get(f"/api/v1/bookings/{bid}").json()["notes"] == "v1"

    # Redo = undo the undo: forward to v2.
    undo_change_id = _changes_for(bid)[-1]["change_id"]
    client.post(f"/api/v1/changes/{undo_change_id}/undo")
    assert client.get(f"/api/v1/bookings/{bid}").json()["notes"] == "v2"
    _cleanup(bid)


def test_undo_unknown_change_is_404():
    assert client.post("/api/v1/changes/nope-not-a-real-id/undo").status_code == 404


# ── Feed cursor ────────────────────────────────────────────────

def test_since_seq_returns_only_newer_events():
    all_now = client.get("/api/v1/changes").json()
    cursor = all_now[-1]["seq"] if all_now else 0
    b = _make_booking()
    newer = client.get("/api/v1/changes", params={"since_seq": cursor}).json()
