...
void main() {
  group('TripForm — create mode', () {
    testWidgets('renders New trip title with Save and Cancel buttons', (tester) async {
      _tallSurface(tester);
    ...
    testWidgets('renders Start date and End date placeholder buttons', (tester) async {
      _tallSurface(tester);
      await _pumpHost(tester, results: []);
    ...
    testWidgets('does not submit when name filled but no dates selected', (tester) async {
      _tallSurface(tester);
      final results = <Map<String, dynamic>?>[];
      await _pumpHost(tester, results: results);
      ...
      // _submit() returns early when _start or _end is null.
      expect(results, isEmpty);
      // Dialog still open (Save not popped).
      expect(find.text('Save'), findsOneWidget);
    });
  ...
  group('TripForm — edit mode', () {
    ...
    testWidgets('renders Edit trip title prefilled with existing data', (tester) async {
      _tallSurface(tester);
      ...
      expect(find.text('2025-07-25'), findsOneWidget);
    });

    testWidgets('save in edit mode returns updated name with original dates', (tester) async {
      _tallSurface(tester);
      final results = <Map<String, dynamic>?>[];
    ...
    testWidgets('save returns payload with all three required keys', (tester) async {
      _tallSurface(tester);
      final results = <Map<String, dynamic>?>[];
      await _pumpHost(tester, existing: existingTrip, results: results);
